...
class TCNModel(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.tcn = nn.Sequential(
            TCNBlock(input_dim, 128, kernel_size=5, dilation=1),
            TCNBlock(128, 128, kernel_size=5, dilation=2),
            TCNBlock(128, 128, kernel_size=5, dilation=4)
        )
    ...
    def forward(self, x):
        ...
        x = self.tcn(x)                # [batch, 128, seq_len]
...
def train_and_evaluate():
    model = TCNModel(FEATURE_DIM, NUM_CLASSES).to(device)
